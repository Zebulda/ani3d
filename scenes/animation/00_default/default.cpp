...
void scene_model::setup_data(std::map<std::string, GLuint>& shaders, scene_structure&, gui_structure& gui)
{
    ...
    cone.uniform.color = { 0.2f,0.8f,0.0f };
    const int N_cone = 40;
    ...
    for(int k=0; k<N_cone; ++k)
    {
        ...
        positions[k] = {x,-0.80,z};
    }


    // Create trunks
    // ************************************** //
    trunk = mesh_drawable(mesh_primitive_cylinder(0.05f, { 0,-0.10,0 }, { 0,0.10,0 }, 20, 20)); // Note that a mesh_drawable can be directly constructed from a mesh
    trunk.uniform.color = { 0.4f,0.2f,0 }; // can set the color (R,G,B) used in the shader
    trunk.shader = shaders["mesh"]; 
...
void scene_model::frame_draw(std::map<std::string, GLuint>& shaders, scene_structure& scene, gui_structure&)
{
    ...
    const vec3 axis_of_rotation = { 0.5,0.5,0 };
    const float angle_of_rotation = time / 3.0f;
    ...
    */
    ...
    const vec3 trunk_placement_diff = {0,-0.10,0};
    for(int k=0; k<N; ++k)
    {
        cone.uniform.transform.translation = positions[k];
        trunk.uniform.transform.translation = positions[k] + trunk_placement_diff;
        draw(cone, scene.camera, shaders["mesh"]);
        draw(trunk, scene.camera);
    }
