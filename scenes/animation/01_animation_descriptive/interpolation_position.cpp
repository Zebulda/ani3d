...
void scene_model::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    int ind0 = idx > 0 ? idx-1 : idx;
    int ind3 = idx < int(N-2) ? idx+2 : idx+1;
    const float t0 = keyframes[ind0].t; // =t_{i-1}
    const float t1 = keyframes[idx  ].t; // = t_i
    const float t2 = keyframes[idx+1].t; // = t_{i+1}
    const float t3 = keyframes[ind3].t; // = t_{i+2}

    const vec3& p0 = keyframes[ind0].p; // = p_{i-1}
    const vec3& p1 = keyframes[idx  ].p; // = p_i
    const vec3& p2 = keyframes[idx+1].p; // = p_{i+1}
    const vec3& p3 = keyframes[ind3].p; // = p_{i+2}
...
static vec3 cardinal_spline_interpolation(float t, float t0, float t1, float t2, float t3, const vec3& p0, const vec3& p1, const vec3& p2, const vec3& p3)
{
    ...
    const vec3 di = coef * (p2 - p0) / (t2 - t0);
    const vec3 di1 = coef * (p3 - p1) / (t3 - t1);

    const float s = (t-t1)/(t2-t1);

    const vec3 p = (2*s*s*s - 3*s*s + 1)*p1 +
    (s*s*s - 2*s*s + s)*di + (-2*s*s*s + 3*s*s)*p2 + (s*s*s - s*s)*di1;
